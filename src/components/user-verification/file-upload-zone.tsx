...
interface FileUploadZoneProps {
  ...
  onUploadError: (file: UploadedFile) => void
  onRemove: () => void
}

function formatFileSize(bytes: number) {
  if (bytes === 0) return 'Uploaded' // For previously uploaded files without size info
...
export function FileUploadZone({
  ...
  const { startUpload, isUploading } = useUploadThing('identityDocument', {
  ...
  // Check if we should show an image thumbnail
  const showThumbnail =
    file?.status === 'success' && file.url && isImageUrl(file.url)
